...
/**
 * The agent-facing description for each tool. Kept here, out of the registration
 * calls in `main()`, so the registrations read as a flat "name → schema →
 * handler" list rather than walls of prose.
 */
const DESCRIPTIONS = {
    search: "Search authentik's API: free-text query over path/operationId/summary/tags. Returns matching operations with their parameter, request, and response schemas. Use this to discover what to call before writing code.",
    execute:
    ...
    docs: "Return the authentik documentation base URLs for THIS instance (version-aware). Prefer these over any hardcoded docs URL: fetch `<docsURL>/llms.txt` (or `<integrationsURL>/llms.txt`), follow the index to the relevant page, then fetch its `.md`.",
} as const;
...
async function main(): Promise<void> {
    ...
    tool<{ content: string }>(
        "validate_blueprint",
        DESCRIPTIONS.validate_blueprint,
        { content: z.string() },
    ...
    tool<{ content: string }>(
        "prepare_apply",
        DESCRIPTIONS.prepare_apply,
        { content: z.string() },
        async (args) => asContent(await tools.prepare(args)),
    );

    tool<Record<string, never>>("docs", DESCRIPTIONS.docs, {}, async () =>
        asContent({
            docsURL,
            integrationsURL,
